...
void writeSTL_binary(int numTriangles, float nodes[][3], int triangles[][3], char* *filename){
...
void writeSTL_ASCII(int numTriangles, float nodes[][3], int triangles[][3], char* *filename){
    FILE *f=fopen(*filename,"w");
    fprintf(f,"solid \n");
    for (int i=0; i<numTriangles; i++){
        float normal[3];
...
int setRingsCurve(float nodes[][3], float x0, float y0, float z0, float r, int n, int ct){
    ...
    for(int ring=0; ring<36; ring++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(ring==35){
                printf("%i: %f / %f / %f\n",i,i_ring[i][2],nodes[ct][2],sin(elb_angle));
            }
            ct++;
        }
    }
...
void makeElbow(float r, int n){

//    36 rings plus two more rings at the sides
    int numNodes = 36*n;
    int numTriangles = 70*n;
    ...
    printf("Set rings curve\n");
    nNode = setRingsCurve(nodes, 0, 0, 0, r, n, nNode);
    ...
    char* filename="elbow.stl";
    writeSTL_binary(numTriangles,nodes,triangles,&filename);
